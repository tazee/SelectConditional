...
#define ATTRa_RANGE1   4
    ...
    CSelectPolysByArea::CSelectPolysByArea()
    {
        ...
        static const LXtTextValueHint conditional_operators[] = { { OPERATOR_LESSTHAN, "less_than" },
                                                                  { OPERATOR_RANGE, "range" },
                                                                  { OPERATOR_GREATERTHAN, "greater_than" },
                                                                  { 0, "=conditional_operators" },
                                                                  0 };
    ...
    LxResult CSelectPolysByArea::tmod_Down(ILxUnknownID vts, ILxUnknownID adjust)
    {
        int  x, y;
        auto viewId = s_v3d.Mouse(&x, &y);
        s_v3d.View(viewId, m_view3D);
        dyna_Value(ATTRa_AREA).GetFlt(&m_area);
        dyna_Value(ATTRa_RANGE0).GetFlt(&m_range0);
        dyna_Value(ATTRa_RANGE1).GetFlt(&m_range1);
        return LXe_TRUE;
    }

    void CSelectPolysByArea::tmod_Move(ILxUnknownID vts, ILxUnknownID adjust)
    {
        ...
        if (m_operator == OPERATOR_RANGE)
        {
            ...
            double amount = (spak->cx - spak->px) * m_view3D.PixelSize();
            amount = (amount * amount) * LXxSIGN(amount);
            range0 = m_range0 - amount * 0.5;
            range1 = m_range1 + amount * 0.5;
            if (range0 < 0.0)
                range0 = 0.0;
            if (range1 < 0.0)
                range1 = 0.0;
        ...
        {
            double area = m_area + (spak->cx - spak->px) * m_view3D.PixelSize();
            if (area < 0.0)
            {
                area = 0.0;
            }
    ...
    void CSelectPolysByArea::tmod_Up(ILxUnknownID vts, ILxUnknownID adjust)
    {
    }
    ...
    void CSelectPolysByArea::tmod_Initialize(ILxUnknownID vts, ILxUnknownID adjust, unsigned int flags)
    {
        ...
        poly.Area(&area);
        at.SetFlt(ATTRa_AREA, area);
        at.SetFlt(ATTRa_RANGE0, area);
        at.SetFlt(ATTRa_RANGE1, area);
    }
    ...
    LxResult CSelectPolysByArea::atrui_UIHints(unsigned int index, ILxUnknownID hints)
    {
        ...
        if (index == ATTRa_AREA || index == ATTRa_RANGE0 || index == ATTRa_RANGE1)
        {
    ...
    public:
        ...
        {
            if (m_operator == CSelectPolysByArea::OPERATOR_RANGE)
            {
                if (area >= m_range0 && area <= m_range1)
                    return true;
            }
